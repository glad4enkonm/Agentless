#!/bin/bash
# Agentless minimal pipeline on the test split with the local gemma model
# (llama.cpp server on :8081, model gemma4-26b-A4B-Q8_0).
# Same proven config as run_gemma_dev.sh; only SPLIT and RESULT_DIR differ.
#   - RESULT_DIR is gemma-test-specific; qwen results (swe-bench-lite-test) untouched
...
SPLIT="test"
RESULT_DIR="results/swe-bench-lite-test-gemma26b"
NUM_THREADS=10
